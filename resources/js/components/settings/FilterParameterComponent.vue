                            ...
                            <table v-if="VariableList.length>0" class="table table-sm table-bordered">
                                ...
                                <tbody>
                                    <tr v-for="(row,index) in VariableList" :key="index">
                                        <td> 
                                            <input type="checkbox" :value=row.id v-model="selectedVariables" id="defaultCheck1">
                                        </td>
                                        <td> {{row.qid}}</td>
                                        <td> {{row.question_text}}</td>
                                    </tr>
...
<script>
    export default {
        ...
        methods:{
            getTableData(){
                axios.get('/settings/'+this.project_id+'/filter_parameters/get_table_data')
                .then(response => {
            ...
            getVariableList(){
                axios.get('/settings/'+this.project_id+'/filter_parameters/get_variable_list')
                .then(response => {
            ...
            saveVariables(){
                ...
                // console.log(newData);
                axios.post('/settings/'+this.project_id+'/filter_parameters/set_variable_list', {'varList':newData})
                    .then(response => {
                    // this.VariableList = response.data;
            ...
            removeVariable(id){
                axios.get('/settings/'+this.project_id+'/'+id+'/filter_parameters/remove_variable')
                .then(response => {
                    this.TableData = response.data;
                    // console.log(response.data);
                });
