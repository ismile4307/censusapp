...
class FilterParametersController extends Controller
{
    public function index($id){
        $project=Project::find($id);
        $my_class=new MyClass();
        $user_activity=$my_class->get_user_role();
        return view('settings.filter_parameters',['project'=>$project,'activity'=>$user_activity]);
    }
    ...
    public function get_variable_list($id){
        ...
        $db_result=DB::select('SELECT id, qid, question_text FROM questions_'.$project_code.' WHERE show_in_filter=0');
        return $db_result;
    }
    ...
    public function remove_variable($id,$qid){
        $project=Project::find($id);
        $project_code=$project->project_code;
        DB::update('UPDATE questions_'.$project_code.' SET show_in_filter = 0 WHERE id = ?', [$qid]);
